...
import { allPosts as allPostsExternal } from "contentlayer/generated";
 ...
 */
export const allPosts = <Keys extends keyof Post>(fields: Keys[], limit?: number) => {
  return allPostsExternal
    .filter((post) => process.env.NODE_ENV === "development" ? true : post.status === "published")
    .sort((a, b) => {
      return compareDesc(new Date(a.publishedAt || new Date()), new Date(b.publishedAt || new Date()));
    })
