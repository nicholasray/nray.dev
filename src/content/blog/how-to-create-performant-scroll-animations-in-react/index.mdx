...
Performant web animations can be tricky to get right. Coupling these animations
with a high-frequency critical event like scrolling increases the stakes. Done
well, they can enhance the user's experience. Done poorly, they can frustrate
users and make your site unusable.
...
```ts showLineNumbers title="useIntersectionObserver.ts" {7-11}
function useIntersectionObserver({
  root = null,
  rootMargin = "0%",
  threshold = 0,
...
```ts showLineNumbers title="useIntersectionObserver.ts" {12-49}
function useIntersectionObserver({
  root = null,
  rootMargin = "0%",
  threshold = 0,
  ...
  useEffect(
    ...
    // an array.  eslint-disable-next-line react-hooks/exhaustive-deps
...
Check out the [demo](https://codesandbox.io/s/tender-river-k6rn1w), but keep in
mind that you won't see the animation if your viewport is less than 640px due to
the media query styles that we added above.
