...
Unfortunately, **35.1% of mobile websites** are falling short of Google's latest
...
<Image
  background
  src={inpFid}
  alt="An interaction timeline consisting of input delay, processing time, and presentation delay portions. First Input Delay only measures the input delay portion while Interaction to Next Paint measures all the portions of an interaction."
  caption="FID only measures the input delay portion of an interaction while INP measures all three components of an interaction."
/>
...
<Image
  background
  src={inpSpinner}
  alt="An interaction timeline showing the interaction latency between a click, tap, or key press and a visual update consisting of a spinner."
...
Most of the time, INP becomes the page's **longest interaction latency**. The
exception is when a user performs over 50 interactions on a page. In this case,
INP ignores the most prolonged interaction for every 50 interactions, hopefully
reducing extreme outliers. Single-page applications (SPAs) are particularly
prone to many interactions since the user doesn't navigate away from the page.
...
<Image
  background
  src={inpComponents}
  alt="Detailed interaction timeline showing that an interaction consists of blocking tasks (input delay), event callbacks like pointerup, mouseup, and click (processing time), and render, paint, and composite (presentation delay). At the end is a visual update."
...
Each of these phases can add time to the interaction's latency.
...
- There may be
  ...
  page you've entered, PSI will try to use data from all pages of the origin
  instead. PSI won't show any data if the origin doesn't have enough data.
...
- **Style and layout**: Matching elements to selectors and calculating an
  element's computed styles and its geometric properties
...
<Image
  background
  src={inpBlockedTask}
  alt="Interaction timeline showing a button click interaction being blocked by a JS task which contributes to a high input delay."
...
initializes. During this time, the browser is parses HTML, evaluates scripts,
renders, and paints.
...
- A single `<script>` tag (without the `type=module` attribute) that loads a
  ...
  evaluates the JavaScript once loaded, this can become problematic if the
...
<Image
  ...
  alt={`Interaction timeline showing an "A" keypress followed by a "B" keypress resulting in a input delay from the execution of event callbacks related to the "A" keypress.`}
  caption='Interactions that happen frequently, like typing "A" into an input box quickly followed by "B", can cause interaction overlap and a high input delay.'
/>
...
- If the interaction involves an animation, only animate `transform` and
...
delay. Fortunately, there are optimizations you can make to ensure this doesn't
happen.
...
the next best option is to **yield to the main thread**. Split up the work into
**smaller tasks** that each take less than **50 ms** to execute and prioritize
the work that is user-facing. The benefits of doing this are two-fold:
...
2. Deferring non-essential work to another task in a `setTimeout` callback
   allows the main thread the opportunity to handle higher priority tasks like
...
like `element.getBoundingClientRect()`, after modifying styles, such as by
adding a class to an element, the browser needs to perform a
...
This becomes particularly expensive when multiple forced synchronous layouts are
invoked in quick succession. When this happens, it's called
...
The last phase that can go wrong is the presentation delay portion that involves
rendering and presenting the visual changes. It also happens to be one of the
hardest phases to optimize.
...
with more than 1,500 elements. The rendering work that the browser must do to
produce a frame correlates with the size of the DOM. Generally, **the larger the
DOM size, the longer it takes for the browser to render updates**.
...
to the rendering tree and will cause some delay, it reduces the rendering costs
unless the user toggles all sections. Wikipedia uses this technique on its
mobile site for most sections on the page.

import hiddenUntilFound from "./_assets/wikipedia-mobile-hidden-until-found-attribute.png";

<Image
  src={hiddenUntilFound}
  alt={`Screenshot of Wikipedia's mobile site showing many collapsed article sections. Chrome DevTools shows that the sections are hidden with a "hidden=until-found" attribute.`}
