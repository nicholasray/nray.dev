...
Around **35% of mobile websites** are falling short of Google's latest web
...
Web Vital on March 2024. Although both metrics measure a page's interactivity
and responsiveness, INP provides a more complete assessment of a user's
experience on a page compared to FID by:
...
1. **Input delay**: Begins when a user clicks, taps, or presses a key and ends
   when the first JavaScript event callback start executing
...
_all_ the way up to the first frame after an interaction, it's a much better
