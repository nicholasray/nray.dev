...
**interactivity** and **responsiveness**. Google will use INP as a user
experience signal that influences your website's search ranking.
...
delay. Fortunately, optimizing your JavaScript will prevent this from happening.
...
For example, this pattern can work well for accordion sections. While the
elements are hidden, they are removed from the browser's rendering tree and
won't contribute to presentation delay. Then, you can programmatically show an
element after an interaction, like clicking on an accordion section. Although
this will add the element back to the rendering tree where it can contribute to
presentation delay, this technique typically reduces the number of elements in
the rendering tree. Wikipedia uses this technique on its mobile site for most
