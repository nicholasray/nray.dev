...
spinner or even the letter within an input field that corresponds to a
keystroke. As long as the interaction results in timely visual feedback that
shows the user _something_ has happened, you can get good INP scores. Just make
sure you use the
...
_all_ the way to the first frame after an interaction, it is much better
...
- A single `<script>` tag (without `type=module`) that loads a huge amount of
  ...
  JavaScript you are loading. Next, consider having multiple script tags so that
  each one kicks off its own _smaller_ task or executing expensive logic in a
...
that takes too long to execute. This results in a long task on the main thread
...
which will improve responsiveness.
...
work** into smaller tasks and prioritizing the work that is user-facing. The
...
and can significantly increase processing time. This can happen when a read and
write are inside a loop, for example:

```js
const sections = documet.querySelectorAll("section");

// Each iteration causes a forced synchronous layout which results in layout
// thrashing.
...
To eliminate forced synchronous layouts and layout thrashing, batch and perform
...
Large presentation delays are often the result of large DOM trees. The rendering
work that the browser must do in order to produce a frame correlates with the
size of the DOM. As a result, in general, the larger the DOM size, the longer it
takes for the browser to render updates.
...
technique on their mobile site for the page's section.
