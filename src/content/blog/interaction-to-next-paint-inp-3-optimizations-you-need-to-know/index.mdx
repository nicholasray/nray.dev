...
prone to responsiveness issues because tasks take longer to execute.
...
The main thread is often the most congested when a page loads and starts up.
Script evaluation, rendering work, and painting commonly occur here.
...
- Having a single `<script>` tag (without `type=module`) that loads a huge
  amount of JavaScript. Since each `<script>` executes a task to evaluate the
  JavaScript once loaded, this can become problematic if the evaluation takes a
  long time since it can become a potential source of input delay. Instead, try
  to reduce the amount of JavaScript you are loading. Also, consider having
  multiple script tags so that each one kicks off its own _smaller_ task that
  can cause less of an input delay.
- Recurring timers
- JavaScript that creates or mutates large DOM trees. Pages with lots of
  elements are often associated with very costly rendering and paint tasks.
