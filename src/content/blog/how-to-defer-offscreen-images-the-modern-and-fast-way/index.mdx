...
This is the technique used by some of the biggest sites in the world,
including [**Medium**](/assets/deferred-offscreen-image/medium.png),
[**LinkedIn**](/assets/deferred-offscreen-image/linkedin.png), and
[**IMDb**](/assets/deferred-offscreen-image/imdb.png). In this article,
...
are using this feature. The rest either load their images during page load
or use JavaScript to lazy-load them. But JavaScript can cause
...
**You should only lazy-load images that are "below the fold" — the part of
the page that is hidden before scrolling.**
...
is more important than the other images. **But only use this attribute for
the largest above-the-fold image on the page.** Prioritizing many images
can lead to bandwidth contention, which defeats the purpose of
prioritization.

Note that [browser support](https://caniuse.com/?search=fetchpriority) for
this attribute isn't good right now, but browsers that don't support it
will ignore it.
