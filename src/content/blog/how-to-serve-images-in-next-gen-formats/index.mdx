...
cover:
  ...
  alt:
    "Browser showing a passing `serve images in next-gen formats` audit."
  position: "left top"
description:
...
formats.**" The audit flags images that could have been at least 8 KiB
...
Safari. AVIF has also seen a steady increase of browser support.
...
especially when combined with other techniques like
[lazy loading images](/blog/how-to-defer-offscreen-images/). You can use
...
- You are likely missing out on (much) smaller file sizes that you might
  get from the less supported AVIF format.
...
You can save multiple image formats using the same tools mentioned above.
For the most optimized image supporting the most extensive set of browsers,
save to **AVIF**, **WebP**, and **JPEG** (or **PNG**) formats.
...
<picture>
  ...
  <!-- If the picture element isn't supported, load
  the JPEG from the <img> element -->
  <img
    width="200"
    height="200"
    alt="A sitting cat."
...
- If the browser supports AVIF, the AVIF loads from the first `<source>`
  element.
- If the browser doesn't support AVIF but supports WebP, the WebP loads
  from the second `<source>` element.
...
- If the browser doesn't support the `<picture>` element
  ([IE11](https://caniuse.com/picture)), the JPEG loads from the `<img>`
  element.
...
is unnecessary**. The server will choose the best format, typically a WebP
or AVIF, and send _that_ back to the user. This is the strategy I'm using
on this site.
...
Finally, dedicated image CDNs like [Cloudinary](https://cloudinary.com/) or
[imgix](https://imgix.com/) are also popular options that use content
negotiation.
...
connection, which can negate the benefits of image optimization. One option
