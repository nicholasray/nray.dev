...
[this demo page](/demos/use-video-instead-of-gif-before/). I've used a
...
optimization can be a pain at scale, I prefer to delegate these
...
`<video>` element, which can serve different formats via the
...
width of the content. The `playsinline` attribute is needed on iOS to
prevent the video from entering fullscreen mode.
...
loaded, which causes the layout shift. What can we do about this?

## Step 4: Add aspect-ratio CSS

Unlike the `<img>` element, we can't just add `width` and `height`
...
a result, we've eliminated the layout shift.
...
Okay, now we have an optimized video that serves different formats
...
<Image
  ...
  alt="WebPageTest waterfall showing the first frame of the video doesn't appear until after the video has been loaded."
/>

This waterfall shows that the video's first frame only appears after
the entire video finishes downloading. Ideally, we want that to render
as quickly as possible to indicate visual progress. As of
...
we can encourage the browser to paint the first frame faster. That
instructs the browser to download and display an image while the video
is loading.

Using FFMpeg, we can use the following command to extract the first
...
We can raise the quality by lowering the `qscale:v` param (1 is the
highest quality, 31 is the lowest quality).

Using Cloudinary, you can set the file extension as a `jpg` and
decrease the file size by adding the `q_10` to the path, reducing the
image's quality.
...
Remember, its only purpose is to indicate visual progress, so it's
okay if the quality is low. To this end, the poster image I've
generated with Cloudinary results in a **4 KB image**.
...
Let's test the [final result](/demos/use-video-instead-of-gif-after/)
and compare it with the GIF.
...
<Image
  ...
  caption="WebPageTest showing a 100 ms improvement in Largest Contentful Paint."
/>
...
- added a low-quality poster image to show visual progress and improve
  LCP
