...
<Aside heading="Explicit width + correct aspect ratio = no layout shift">
  To prevent layout shifts, set an explicit width like `width: 100%`
  (as done here) or `width: 500px` on the video element **and** set
  the `aspect-ratio`. If you only apply an aspect ratio, the video
  will default to a [width of 300px and height of
