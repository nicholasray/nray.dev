...
converting video. You can use the following command to convert to MP4:
...
By converting the GIF to video with FFMpeg, the file size reduced by
**95%** when converted to MP4 and by **97%** when converted to WebM.
...
The `style` attribute lets the video expand to the width of the
content. The `playsinline` attribute is needed on iOS to prevent the
...
quickly as possible to indicate visual progress. As of
...
You can raise the quality by lowering the `qscale:v` param (1 is the
...
aspects of your website's performance, including load time, Largest
Contentful Paint and page weight.
