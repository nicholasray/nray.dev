...
<Aside heading="Just use lossless compression for everything?">
  **Typically** (but not always), using lossy compression results in
  **smaller file sizes** with acceptable image quality, so that is
  ...
  where using lossless actually results in a smaller file size than
  lossy.
</Aside>
...
balance** of **small file size** and **excellent image quality**. A
small file size means little if the image is too blurry or pixelated
to see. And perfect image quality is irrelevant if it takes too long
to download. In either case, the user doesn't see the image. There
...
multiple files with either lossy or lossless compression.
...
2. Ask Cloudinary to automatically pick the best compatible format
   (`f_auto`) and quality (`q_auto`) while resizing the image to a
   width of 500 px (`w_500`)
3. Fetch the original image from `https://upload.wikimedia.org...`
