...
<Aside heading="Be careful with the `src` attribute">
  Using a domain separate from your own (like
  https://res.cloudinary.com) for the `src` attribute causes the
  browser to set up a new connection, which can easily take over 500
