...
<Aside heading="Be careful with the `src` attribute">
  Using a separate domain (like https://res.cloudinary.com) in the
  `src` attribute causes the browser to create a new connection, which
  can easily take over 500 ms. For the fastest load times, try to [set
  up a
  proxy](https://timkadlec.com/remembers/2020-11-17-netlify-proxy-requests/)
  that avoids the extra connection cost. In 2022, [more than half of
  images](https://almanac.httparchive.org/en/2022/media#cross-domain-image-hosts)
  used a cross-domain host, resulting in an delay.
</Aside>
