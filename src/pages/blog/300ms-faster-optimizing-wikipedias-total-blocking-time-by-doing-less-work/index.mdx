...
have a metric called "Total Blocking Time" (TBT) that measures the sum of this
blocking portion — the portion in excess of 50ms — of all long tasks between
[First Contentful Paint](https://web.dev/fcp/) and
[Time to Interactive](https://web.dev/tti/).

Google
[recommends a TBT that is less than 200 milliseconds](https://web.dev/tbt/#what-is-a-good-tbt-score)
...
collection, long JavaScript execution is often the culprit of TBT problems.

Profiling Wikipedia's mobile site revealed approximately 200ms spent in an
`_enable` method responsible for setting up the mobile site's section
expansion/collapsing behavior. This method contained code that, using jQuery's
insidious `.on` method, added a click event listener to nearly every link in the
content so that the corresponding section would open if the link contained a
hash fragment. For long articles like the
...
JavaScript and free up nearly 200ms from the main thread.

When profiling, always check where the most time is being spent. Then see if you
can optimize the code responsible for that time easily.
