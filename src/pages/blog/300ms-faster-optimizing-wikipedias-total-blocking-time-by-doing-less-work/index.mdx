...
Profiling Wikipedia's mobile site revealed a long time — approximately 200ms —
spend in an `_enable` method responsible for setting up the mobile site's
section expansion/collapsing behavior. This method contained code that added a
click event listener to nearly every link in the content. Worse yet, long
articles like the [United States](https://en.wikipedia.org/wiki/United_States)
article contained over 4,000 links!:
