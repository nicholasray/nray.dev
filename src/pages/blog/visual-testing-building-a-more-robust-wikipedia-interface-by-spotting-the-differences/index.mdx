...
number of innocuous differences, there are times when I don't know _how many_
visual changes a code change might cause. And contrary to the original game,
where the consequences for not catching the differences might result in you not
scoring high enough to appear on a leaderboard somewhere, the effects of not
catching accidental changes can include degradations to the user experience,
loss of functionality, significant amount of time spent fixing the issues, or
worse.
...
**visual testing** to help develop one of our most ambitious projects.
...
fixing those regressions, and then having similar regressions occur in as little
as a few weeks.
...
that happened, the table of contents menu button should be at the top of the
...
changes in code review. In addition, a lot of time usually spent manually
testing visible changes on a local server was replaced by our visual tests doing
that work for us in a matter of minutes. As a result, confidence in releases
also increased.

The benefits we saw from visual testing continue today. We frequently catch
unintended visual changes with our tests and found visual testing valuable
enough to continue using it in our workflow. I have a hard time reviewing any
code without it.

We still have a long way to go, but we are improving. Our "desktop" test suite
captures 140 screenshots and tests five different viewport widths. Yet, we still
have visual regressions that slip into production. However, many of these result
from _not_ having the necessary visual tests in place for specific scenarios
...
So we will continue increasing coverage of our visual test suite with more tests
and more scenarios. With each new visual test we write, we develop an unfair
advantage in the "Spot the Difference" game. And the difference between where we
were before visual testing and now feels like a step in the right direction.
