...
number of innocuous differences, there are times when I can't tell _how many_
visual changes a code change might cause. And contrary to the original game,
where the consequences for not noticing the differences might result in you not
...
1. Comparing the `HEAD` of the `master` branches with a
   [Gerrit patch](https://gerrit.wikimedia.org/). This is primarily used in code
   review.
