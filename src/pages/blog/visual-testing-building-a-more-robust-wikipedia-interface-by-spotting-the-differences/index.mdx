...
<Image
  ...
  caption="A grid showing 8 different states that the table of contents, page tools, and the main menu could have."
/>
...
- Because our tests compare each pixel, they can flag changes that might
  otherwise be imperceptible to humans and cause the results of a report to be
  noisy. Usually, adjusting the difference threshold to ignore subtle changes
...
most of these result from _not_ having the necessary visual tests in place for
specific scenarios rather than a failure of the existing tests.
