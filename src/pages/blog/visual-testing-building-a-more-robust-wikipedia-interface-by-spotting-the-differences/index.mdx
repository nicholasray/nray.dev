...
_insert spot the difference pic here_
...
_insert video of wikipedia experience here_
...
would move to a less attractive location. Sometimes its size would look
disproportionately big or small.
...
It's not perfect. We have spent a nontrivial amount of time trying to make flaky
tests (typically those that rely on asynchronous logic), less flaky.
[Intentionality and attribution](https://phabricator.wikimedia.org/T320303) of
the visual changes can also be a problem. However, it's an improvement from
where we were.

To date, our "desktop" test suite, contains tests that capture a total of 140
screenshots across 5 different viewport widths. We still have visual regressions
that slip into production, but many of these are the result of _not_ having the
necessary visual tests in place for certain scenarios rather than a failure of
the tests that have been written.

With each new visual test we write, I feel we are developing an unfair advantage
in the "spot the difference" game. And it feels good.
