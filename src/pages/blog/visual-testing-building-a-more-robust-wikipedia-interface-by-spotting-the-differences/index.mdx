...
It involves trying to find slight differences between two very similar images.
When I review code in a nontrivial patch or pull request that changes a
website's HTML, CSS, or JavaScript, I'm playing this game. Sometimes I win.
Other times, I lose.

Yet, unlike the original game, where the player tries to identify a known number
of harmless differences, there are instances when it is difficult to determine
the number of visual changes a code change might cause. Additionally, failing to
spot these differences in code can result in a degraded user experience, loss of
functionality, significant time spent resolving the issues, or worse.
...
At that point, we had built and released several features of our new skin,
...
glass icon would move to a less attractive location as if an invisible magnet on
the page was pulling it.
...
Also, our UI was only getting more complex. For example, we wanted certain
elements only to be visible when logged in and only when the viewport was above
a specific breakpoint. Center the content on the page, but only when the table
of contents isn't pinned next to it. Speaking of the table of contents, when the
user clicks its "hide" button, it should collapse into the page toolbar, but
only if the user hasn't scrolled past the page's first heading to make the
sticky header appear. Otherwise, it should be in the sticky header. Unless, of
course, the viewport was at a small width. If that happens, the table of
contents menu button should be at the top of the viewport but without the sticky
header showing.
...
more and more frequently. We needed something more than unit and integration
tests could provide. Something that could test what users _see_.
...
- Because our visual tests compare each pixel, they can flag changes that might
  ...
  Usually, adjusting the difference threshold in the tests to ignore subtle
  changes alleviates this problem.
- Writing tests that capture the page at the right moment can be a pain. Visual
  ...
  [wait for idle moments in the network and main thread](https://github.com/wikimedia/pixel/blob/474fd24e81ddb39f60b09ca2ed69e67c3d4f8774/src/engine-scripts/puppet/onReady.js#L51-L54)
...
coverage, we still have visual regressions that slip into production. However,
most of these occur due to the need for more visual tests for specific scenarios
