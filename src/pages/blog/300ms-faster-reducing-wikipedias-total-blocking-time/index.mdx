...
with the page by clicking a button when JavaScript began to execute. Because
JavaScript is single-threaded, the user would need to wait this amount of time
before the browser could execute the click handler and respond to that
interaction with a visual update. And any interaction that takes longer than
