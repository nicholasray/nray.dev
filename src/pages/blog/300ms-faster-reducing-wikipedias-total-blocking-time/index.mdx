...
600ms may not sound like a long time, but imagine if a user tried to interact
with the page by clicking a button when JavaScript began to execute during page
load. Because JavaScript is single-threaded, the user would need to wait for the
following steps to finish before they saw a visual update:

1. The JavaScript to complete
2. The relevant click handler to complete
