...
on low-end devices and block interactions from the user.
...
that interaction with a visual update. And users _can_ perceive any interaction
...
<Image
  ...
  caption="If the user clicks a button during the execution of a long task, they need to wait for the long task to finish before the click handler executes that produces a visual update."
/>
...
When I profiled Wikipedia's mobile site, I found approximately **200ms** spent
in an `_enable` method responsible for initializing the mobile site's section
...
2. In the `onClickImage` method, I used the `ev.target.closest(selector)` API to
   ...
   thumbnails. If it did, we handle the event.

But what were the results of this work?

## Conclusion

The work from step 1 and step 2 were released to production in two deploys (step
1 was deployed first). Wikipedia's
