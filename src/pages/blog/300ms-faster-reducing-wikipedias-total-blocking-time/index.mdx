...
600ms may not sound like a long time, but imagine if a user tries to interact
with the page by clicking a button when the 600ms JavaScript began executing
during page load. Because JavaScript is single-threaded, the user would need to
wait for the following steps to finish before they see a visual update:

1. The 600ms JavaScript task to complete
2. The relevant click handler task to complete
3. The browser to perform the necessary rendering steps to visually update the
   page.

Each of these takes time, and any interaction that takes longer than
[100ms](https://web.dev/rail/#response-process-events-in-under-50ms) can be
perceived as **slow**.
...
Google considers any task on the browser's
...
developed a metric called "Total Blocking Time" (TBT) that measures this.
...
amount of time after 50ms in each long task.
...
2. The second task doesn't contribute to TBT at all since it is less than 50ms
   and therefore is not a long task.
3. The 100ms task is **50ms** longer than 50ms so contributes **50ms** to TBT.

Since TBT is the sum of the time exceeding 50ms in each long task, the TBT for
...
To decrease TBT, you need to either:

1. Do less work on the main thread between First Contentful Paint and Time to
   Interactive
2. Do the same amount of work, but break up long tasks into smaller tasks that
   don't exceed 50ms
...
site's section expansion and collapsing behavior. And much of the time was spent
in an insidious call to jQuery's `.on("click")` method.
...
over 4,000 links, leading to over 200ms of execution time on low-end devices.

Worse yet, this behavior was primarily redundant. The following code that
listened to the `hashchange` event already called the same method that the click
event listener called. Unless the window's location already pointed at the
link's destination, clicking a link called the `checkHash` method twice — once
for the link click event handler, and once more for the `hashchange` handler.
...
click to a thumbnail would open a media viewer:
...
event delegation is often more efficient than adding an event listener to many
elements, especially when user-generated content could add an unknown number of
elements. It takes advantage of
...
We deployed the work from step 1 and step 2 to production in two deploys — step
1 followed by step 2.

Wikipedia's
[synthetic performance tests](https://developer.mozilla.org/en-US/docs/Web/Performance/Rum-vs-Synthetic#synthetic_monitoring)
showed that TBT decreased by about 200ms after the first deployment and improved
by about 80ms after the second deployment when testing on a Moto G (5) phone.
Together, this work reduced TBT by almost **300ms**.
...
small pieces of code to make a significant impact.
