...
The example above shows the button's "Dark" label when light mode is active. The
`dark:hidden` class hides the "Dark" label when dark mode is active, while the
"Light" label is shown by the `dark:block` class.
