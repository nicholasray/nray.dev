...
at the time of this writing. Nevertheless, the concepts presented here apply to
many dark mode implementations.
...
If you inspect the styles of the `<div>` above, you'll see that the `dark:`
prefix adds a wrapping `prefers-color-scheme` media query around the
`background-color` property so that it only becomes active when the user has
enabled dark mode on their device:
...
But what if you wanted to add a toggle button to your website that allowed users
to switch between themes independent of their system's preference? Instead, you
can toggle a `dark` class on the `html` element when the user toggles the theme.
...
Now, we need some JavaScript that makes clicks to the button toggle the `dark`
...
Finally, it would cool if the website's theme color changed when the user
