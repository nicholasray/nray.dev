...
meta tag — most commonly found on mobile devices — can make your website's theme
...
The example above shows the button's "Dark" label when light mode is active.
When dark mode is active, the "Light" label is shown by the `dark:block` class,
while the "Dark" label is hidden by the `dark:hidden` class.
