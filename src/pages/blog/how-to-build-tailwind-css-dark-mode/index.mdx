...
you'll see that it adds a wrapping `prefers-color-scheme` media query around the
gray background style so that it only becomes active when the user has enabled
dark mode on their device:
