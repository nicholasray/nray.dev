...
that. The `theme-color` meta tag suggests a color that the browser should for
its UI. Browser's that [support](https://caniuse.com/?search=theme-color) this
meta tag — most commonly found on mobile devices — can help make your website's
...
When the user toggles a theme, what should happen when they reload the page or
go to another page on your site? For the best user experience, save the user's
theme choice so that their setting remains or "persists." Of course, you'll need
to save their choice in the browser to do this. You can use a `cookie` or
`localStorage`, but `localStorage` is commonly used and easier to implement.
...
clicks the button, we should add the `dark` class to the `html` element:
...
The next step is adding persistence so that the user's selected theme stays
...
Like in the `<head>`, we use `localStorage` to handle persistence. But instead
of retrieving the theme from `localStorage`, we save it.
