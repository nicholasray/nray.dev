...
that the `dark:` prefix adds a wrapping `prefers-color-scheme` media query
...
toggle a `dark` class on the `html` element. Tailwind calls this the `class`
strategy.
...
<Image
  ...
  caption="Adding a `color-scheme` meta tag changes the default color of form controls like checkboxes."
  shadow
/>
