...
result, it is not as prone to jank. There is a slight catch, though. The CSS
...
function useIntersectionObserver({
  ...
  useEffect(
    () => {
      // Return early if the callback has already been executed while the
      // element was intersected, if we don't have a reference to the element
      // yet, if the browser doesn't support IntersectionObserver or if the
      // callback has already executed while the element was intersected.
      if (
        !ref ||
...
media query, the end point of the animation renders, and the transition will be
inert when the user has enabled the setting.
 ...
 * which serves as a proxy for devices with more power and for users who haven't
 * set the "prefers reduced motion" setting on their OS.
 */
